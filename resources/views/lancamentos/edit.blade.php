            ...
            <form action="{{ route('lancamentos.update', $lancamento) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="servidor_id" id="servidor_id" class="form-control @error('servidor_id') is-invalid @enderror" required>
                            ...
                            @foreach ($servidores as $servidor)
                                <option value="{{ $servidor->id }}" @selected($lancamento->servidor_id == $servidor->id)>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="evento_id" id="evento_id" class="form-control @error('evento_id') is-invalid @enderror" required>
                            ...
                            @foreach ($eventos as $evento)
                                <option value="{{ $evento->id }}" data-exige-dias="{{ $evento->exige_dias ? 1 : 0 }}" data-exige-valor="{{ $evento->exige_valor ? 1 : 0 }}" data-exige-observacao="{{ $evento->exige_observacao ? 1 : 0 }}" data-exige-porcentagem="{{ $evento->exige_porcentagem ? 1 : 0 }}" @selected($lancamento->evento_id == $evento->id)>
                ...
                <div class="row mb-3" id="dias-container" style="display: none;">
                    <div class="col-md-6">
                        ...
                        <input type="number" name="dias_trabalhados" id="dias_trabalhados" class="form-control @error('dias_trabalhados') is-invalid @enderror" min="0" value="{{ old('dias_trabalhados', $lancamento->dias_trabalhados) }}">
